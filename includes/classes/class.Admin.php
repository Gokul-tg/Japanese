...
function getActionitemById($from,$to)
 	...
 	function add_meeting()
	{
		...
		$ExeSELECTQuery= $this->ExecuteQuery($SelQuery,"norows");
		if(!$ExeSELECTQuery)
		{
		   ...
		   // $action=$_REQUEST['Actionitem'];
		   	$action =  $_REQUEST['ActionItemId'];
			$dependent=  $_REQUEST['DependentTaskId'];
			$dept=$_REQUEST['DepartmentId'];
			$deptuser=$_REQUEST['DepatuserId'];
			$status=$_REQUEST['statusid'];
			//$dateloop=explode('/',$_REQUEST['datepickerid']);
			//$datepick=$dateloop[2]."-".$dateloop[0]."-".$dateloop[1];
			$datepick=$_REQUEST['datepickerid'];
			$countaction= count($action);
			$countdependent= count($dependent);
			$countdept= count($dept);
			$countdeptuser= count($deptuser);
			$countstatus= count($status);
			$countdatepick= count($datepick);
				...
				echo $InsQuery	= "INSERT INTO `tbl_actions`
											    (
											       `MeetingId`,
											       `ActionItem`,
												   `DependentTask`,
		...
		/* ============== mail to Task persions ======================*/
			$sqlmail="SELECT Email from tbl_users where Id='".$deptuser[$i]."'";
	...
	$message='<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	 ...
	 @mail($to_email,$subject,$message,$headers);
		  ...
		  $SELECTemail="SELECT Email from tbl_users where Id in ({$part})";
	          $reslut=mysql_query($SELECTemail);
...
function GetForms()
	{
	global $objSmarty;
	$sql="select * from tbl_forms where Status='1'";
	$Result	= $this->ExecuteQuery($sql, "select");
	$objSmarty->assign("form", $Result);
	}
